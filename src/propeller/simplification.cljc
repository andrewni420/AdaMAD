...
(defn auto-simplify-plushy
  ...
  [plushy error-function {:keys [simplification-steps training-data simplification-k simplification-verbose?]
                          :or {simplification-steps 100
                               simplification-k 20}
                          :as argmap}]
    ...
    (loop [step 0 curr-plushy plushy]
      (if (< simplification-steps step)
        ...
        (let [new-plushy (delete-k-random (rand-int simplification-k) curr-plushy)
              ...
              new-better? (every? identity (map <= new-plushy-errors initial-errors))]
          (recur (inc step)
                 (if new-better? new-plushy curr-plushy)))))))