...
(def-instruction
  ...
  ^{:stacks #{:float}}
  (fn [state]
    (make-instruction state identity [:float] :float)))

;;logarithm
;; If argument is not positive, returns 0.0
(def-instruction
  :float_log
  ^{:stacks #{:float}}
  (fn [state]
    (make-instruction state #(float (if (<= % 0) 0 (math/log %))) [:float] :float)))
...
(def-instruction
  ...
  ^{:stacks #{:float}}
  (fn [state]
    (make-instruction state #(float (if (zero? %2) 0 (/ %1 %2))) [:float :float] :float)))
