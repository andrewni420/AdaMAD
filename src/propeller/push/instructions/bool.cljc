...
;; Pushes the logical INHIBIT of the top three BOOLEANs
(def-instruction
  :boolean_inh
  ^{:stacks #{:boolean}}
  (fn [state]
    (make-instruction state #(and %1 %2 %3) [:boolean :boolean :boolean] :boolean)))
...
;; Pushes the logical BUFFER of the top BOOLEAN
(def-instruction
  :boolean_buf
  ^{:stacks #{:boolean}}
  (fn [state]
    (make-instruction state identity [:boolean] :boolean)))
...
(def-instruction
  ...
  ^{:stacks #{:boolean}}
  (fn [state]
    (make-instruction state #(not (and %1 %2)) [:boolean :boolean] :boolean)))
...
(def-instruction
  ...
  ^{:stacks #{:boolean}}
  (fn [state]
    (make-instruction state #(not (or %1 %2)) [:boolean :boolean] :boolean)))
...
;; Pushes the logical XNOR of the top two BOOLEANs
(def-instruction
  :boolean_xnor
  ^{:stacks #{:boolean}}
  (fn [state]
    (make-instruction state #(or (and (not %1) (not %2))
                                 (and %1 %2))
                      [:boolean :boolean]
                      :boolean)))
