...
(defn assign-indices-to-data
  ...
  ((:mapper argmap) (fn [data-map index]
...
(defn initialize-case-distances
  ...
  ((:mapper argmap) #(assoc (if (map? %) % (assoc {} :data %)) :distances (into [] (repeat (count training-data) (if (coll? population-size) (first population-size) population-size)))) training-data))
